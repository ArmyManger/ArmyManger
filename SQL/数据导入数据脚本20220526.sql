...
sortNo,createTime,updateTime,valid,deleted,createrId,createrName)
 ...
 --同步停车数据
 INSERT INTO bme_import.dbo.t_Park_Pay_Log_Test(wechatID,orderNo,totalMoney,ketuoOrderNo,carNo,mall_id,openID,member_id,pay_useragent,
 sortNo,createTime,updateTime,valid,deleted,description,createrID,createrName)
 select null,pk.线上车场订单号,pk.应付金额_元,pk.线下车场订单号,pk.车牌号,ISNULL(mall.id,0),'',ISNULL(pls.ID,0),pk.支付方式,
 1000,GETDATE(),GETDATE(),1,0,'',0,'Import' from  bme_import.dbo.ParkFeeOrder pk
 left join #member_pools pls on pk.会员卡号 = pls.memberNo 
  left join saas_MainData.dbo.t_Mall mall on mall.mallName = pls.mallName 

--同步停车发票
 INSERT INTO bme_import.dbo.t_Invoice_Log_Test(purchaserTaxNo,title,invoiceCode,invoiceNo,sellerTaxNo,paperDrewDate,amountWithoutTax,taxAmount,
status,sortNo,createTime,updateTime,valid,deleted,description,createrID,createrName,memberNo,
orderNo,invoiceType,taxRate,mallId,buyerName,casherName,itemBusiness,itemName)
select '',tl.开票业务,'',tl.业务开票单号,tl.税号,tl.开票时间,tl.合计开票金额_元,0, tl.开票状态,
1000,GETDATE(),GETDATE(),1,0,'',0,'Import',pls.CodeNo,tl.发票中心订单号,10,0,mall.id,
'',tl.开票抬头,tl.开票业务,tl.开票项目  from bme_import.dbo.t_invoice1 tl
inner join bme_import.dbo.t_Park_Pay_Log_Test pk on tl.业务订单号 = pk.orderNo
 left join #member_pools pls on pk.ketuoOrderNo = pls.memberNo 
  left join saas_MainData.dbo.t_Mall mall on mall.mallName = pls.mallName 
